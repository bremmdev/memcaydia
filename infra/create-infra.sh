...
#      with 403/404 answered by /index.html as a 404 (see infra/README.md)
...
EOF
  ...
  # Both error responses return 404: a missing key on the private bucket comes
  # back as 403 AccessDenied (no s3:ListBucket), but it really means "not found".
  # Valid client routes are already rewritten to /index.html (200) by the
  # url-rewrite.js CloudFront function, so anything reaching here is genuinely
  # missing — 404 rather than 200 keeps that honest for crawlers and monitoring.
  # Serving /index.html only makes the app render its own NotFound page.
